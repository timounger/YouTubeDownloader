...
LOG_MSG_FORMAT_WO_LINENO_WO_THREADS = "%(asctime)s [%(name)s][%(levelname)s] %(message)s"
LOG_MSG_FORMAT_WO_LINENO_W_THREADS = "%(asctime)s [%(name)s][%(threadName)s][%(levelname)s] %(message)s"
LOG_MSG_FORMAT_W_LINENO_WO_THREADS = "%(asctime)s [%(name)s:%(lineno)d][%(levelname)s] %(message)s"
LOG_MSG_FORMAT_W_LINENO_W_THREADS = "%(asctime)s [%(name)s:%(lineno)d][%(threadName)s][%(levelname)s] %(message)s"
...
def get_format(line_no: bool = False, threads: bool = False) -> str:
    ...
    if not line_no and not threads:
        msg_format = LOG_MSG_FORMAT_WO_LINENO_WO_THREADS
    elif not line_no and threads:
        msg_format = LOG_MSG_FORMAT_WO_LINENO_W_THREADS
    elif line_no and not threads:
        msg_format = LOG_MSG_FORMAT_W_LINENO_WO_THREADS
    else:  # line_no and threads
        msg_format = LOG_MSG_FORMAT_W_LINENO_W_THREADS
    return msg_format
...
def init_console_logging(level: int, threads: bool = False) -> None:
    ...
    date_format = LOG_DATE_FORMAT if (level != logging.DEBUG) else None
    stream_handler.setFormatter(ColorFormatter(line_no=line_no, threads=threads, date_format=date_format))
...
class ColorFormatter(logging.Formatter):
    ...
    @param fmt : [optional] override format with a complete custom log format -> ignores all other parameters
    ...
    @param date_format : [optional] date format
    """
    fg_grey = "\033[90m"
    fg_yellow = "\033[33m"
    fg_red = "\033[31m"
    ...
    def __init__(self, fmt: str | None = None, line_no: bool = True, threads: bool = False, date_format: str | None = None, **kwargs: Any) -> None:
    ...
    def format(self, record: LogRecord) -> str:
        """!
        @brief Override format method of logging.Formatter to use colored formatting.
        @param record : LogRecord instance to format with color codes
        @return formatted string with ANSI color codes based on log level
        """
    ...
    def formatException(self, ei: tuple[type[BaseException], BaseException, TracebackType | None] | tuple[None, None, None]) -> str:
        """!
        @brief Override formatException method of logging.Formatter to use critical color formatting.
        @param ei : exception info
        @return formatted exception log record
        """
        result = super().formatException(ei)
        return self.bg_red + result + self.reset
