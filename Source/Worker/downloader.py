...
class DownloadThread(threading.Thread):
    ...
    def run(self):
        ...
        """
        self.gui.o_status.config(text="Analysiere URL...", fg="blue")
        i_choice = self.gui.o_format_choice.get()
        s_url = self.gui.o_url_choice.get()
        if  i_choice != 0:
            l_url = []
            s_subfolder_name = ""
            ...
            for i, s_url in enumerate(l_url, 1):
                ...
                self.gui.o_titel.config(text=s_text,fg="orange")
                b_valid_url = False
                try:
                    ...
                    self.gui.o_status.config(text="Ungültige URL!",fg="red")
                if b_valid_url:
                    self.clear_data()
                    s_filename = None
                    if i_choice == 1:
                    ...
                    try:
                        ...
                        if i_choice == 3 and B_MP3_CONVERT:
                            ...
                            self.gui.o_status.config(text="MP3 erstellt!", fg="green")
                    except: # pylint: disable=bare-except
                        self.gui.o_status.config(text="Dieses Video kann nicht heruntergeladen werden!",\
                                              fg="red")
        else:
            self.gui.o_status.config(text="Bitte Format angeben!",fg="red")
        self.gui.o_download_button["state"] = "normal"
    ...
    def progress_callback(self, _stream, _chunk, bytes_remaining):
        ...
        if not self.b_first_callback_call:
            ...
            if self.f_time_stamp != 0: # set calculate time only for second call
                ...
                self.gui.o_status.config(text=f'Download läuft... noch {i_remaining_seconds}sek', fg="blue")
            ...
            for _ in range(i_percent_diff):
                self.gui.o_progress.step()
            self.gui.style.configure('text.Horizontal.TProgressbar', text=f'{i_percent}%')
