...
class DoxyPyChecker:
    ...
    @param exclude_folders : list of folder names to exclude from check
    @param print_checked_files : status if checked files should print
    """

    def __init__(self, path: str | None = None, exclude_folders: list[str] | None = None, print_checked_files: bool = True):
    ...
    def run_check(self) -> list[str]:
        ...
        # print result
        if findings:
            log.warning("Found %s Function with Warnings in %s files.", len(findings), len(py_files))
    ...
    def get_files(self) -> list[str]:
        ...
        for root, dirs, files in os.walk(self.path):
            dirs[:] = [d for d in dirs if os.path.basename(d) not in self.exclude_folders]
            for f in files:
    ...
    def get_doc_params(self, docstring: str, findings: list[str]) -> set[str]:
        ...
        for line in docstring.splitlines():
            if line.lstrip().startswith(PARAM_DOC_PREFIX):
                ...
                if len(param_parts) >= 2:  # minimum two names: @param, param_name
    ...
    def check_function(self, func_def: ast.FunctionDef | ast.AsyncFunctionDef, class_docstring: str | None = None) -> list[str]:
        """!
...
if __name__ == "__main__":
    CHECK_TYPING = False
    doxy_checker = DoxyPyChecker(path="../../Source", print_checked_files=True)
    args = doxy_checker.get_cmd_args()
    if args.path:
        doxy_checker.path = args.path
